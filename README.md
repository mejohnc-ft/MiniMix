...
`scripts/verify-live-voice-mvp.sh release` is the safe orchestrator for the final voice proof. Default mode is check-only, does not record, and uses the no-panel LaunchServices packaged permission check. After stable signing and permissions are ready, run `scripts/verify-live-voice-mvp.sh release --run-live` for the lowest-focus live proof.
