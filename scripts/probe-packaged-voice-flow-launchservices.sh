...
sound="$(mktemp -t minimix-packaged-voice-flow.XXXXXX).caf"
output_file="$(mktemp -t minimix-ls-voice-flow.XXXXXX)"
build_log="$(mktemp -t minimix-ls-voice-flow-build.XXXXXX)"
...
if [[ ! -d "$app" ]]; then
  echo "packagedVoiceHarness ready=false authoritativeForPackagedApp=true reason=missing app bundle" >&2
  exit 3
fi
...
if [[ ! -s "$output_file" ]]; then
  kill "$open_pid" >/dev/null 2>&1 || true
  echo "packagedVoiceHarness ready=false authoritativeForPackagedApp=true reason=no harness output from LaunchServices app run" >&2
  exit 65
fi

status="$(cat "$output_file")"
status="${status/voiceHarness/packagedVoiceHarness authoritativeForPackagedApp=true ready=true}"
printf '%s\n' "$status"
