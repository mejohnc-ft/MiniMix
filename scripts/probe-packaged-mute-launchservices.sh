...
sound="$(mktemp -t minimix-packaged-mute.XXXXXX).caf"
output_file="$(mktemp -t minimix-ls-mute.XXXXXX)"
build_log="$(mktemp -t minimix-ls-mute-build.XXXXXX)"
...
if [[ ! -d "$app" ]]; then
  echo "packagedMuteHarness ready=false authoritativeForPackagedApp=true reason=missing app bundle" >&2
  exit 3
fi
...
if [[ ! -s "$output_file" ]]; then
  kill "$open_pid" >/dev/null 2>&1 || true
  echo "packagedMuteHarness ready=false authoritativeForPackagedApp=true reason=no harness output from LaunchServices app run" >&2
  exit 65
fi

status="$(cat "$output_file")"
status="${status/muteHarness/packagedMuteHarness authoritativeForPackagedApp=true ready=true}"
printf '%s\n' "$status"

if [[ "$status" == *"ready=true"* &&
      "$status" == *"activeAfterMute=1"* &&
      "$status" == *"muted=true"* &&
      "$status" == *"gain=0.0"* &&
      "$status" == *"activeAfterUnmute=0"* &&
      "$status" == *"tapCount=0"* &&
      "$status" == *"activeTapID="* &&
      "$status" == *"activeAggregateDeviceID="* &&
      "$status" == *"sessionsAfterUnmute=0"* ]]; then
  exit 0
fi
