...
sound="$(mktemp -t minimix-packaged-voice-shutdown.XXXXXX).caf"
output_file="$(mktemp -t minimix-ls-voice-shutdown.XXXXXX)"
build_log="$(mktemp -t minimix-ls-voice-shutdown-build.XXXXXX)"
...
if [[ ! -d "$app" ]]; then
  echo "packagedVoiceShutdownHarness ready=false authoritativeForPackagedApp=true reason=missing app bundle" >&2
  exit 3
fi

open -g -W -n "$app" --args \
  --voice-shutdown-harness \
...
if [[ ! -s "$output_file" ]]; then
  kill "$open_pid" >/dev/null 2>&1 || true
  echo "packagedVoiceShutdownHarness ready=false authoritativeForPackagedApp=true reason=no harness output from LaunchServices app run" >&2
  exit 65
fi

status="$(cat "$output_file")"
status="${status/voiceShutdownHarness/packagedVoiceShutdownHarness authoritativeForPackagedApp=true ready=true}"
printf '%s\n' "$status"
